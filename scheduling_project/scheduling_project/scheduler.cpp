...
// !!!배운점!!!
// method 안에서 생성자로 객체를 생성하면 method가 끝날때 해제된다. 그러므로 method안에서 new연산을 한번더 해줘야 한다.
...
ProcessControlBlock* Scheduler::JobPop()
{
    ...
    if (ready_front == nullptr)
        return temp;

    while (true) //nullptr접근 안 되?
    {
        if (temp == nullptr)
            return job_front;
        else if (temp->GetPID() != ready_front->GetPID())
            break;
        else if (temp->GetPID() == ready_front->GetPID())
...
void Scheduler::ReadyPush(ProcessControlBlock& PCB)
{
    ...
    //
    ProcessControlBlock* iterator = ready_front;
    while (iterator != nullptr)
    {
        if (iterator->GetPID() == PCB.GetPID())
            return;
...
void Scheduler::CpuProcess(ProcessControlBlock& PCB)
{
    ...
    else if(PCB.HaveIO() == IOStatus::no || PCB.HaveIO() == IOStatus::done)
    {
...
void Scheduler::DeleteJobQueueItem(ProcessControlBlock& PCB)
{
    ...
    while (iterator != nullptr)
    {
        if (iterator->GetPID() == PCB.GetPID())
        {
            ...
            job_front = iterator->GetNextPointer();
            job_length--;
            delete temp_del;
            return;
        }
