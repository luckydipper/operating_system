...
int OptimalNumPagefault(const deque<int>& reference_string, const int& num_frame)
{
	...
	for (int index = 0; index < reference_string.size(); index++)
	{
		...
		{
			num_page_fault++;
			int extract_index = ChooseExtractIndex_(reference_string, ram_frames, index);
...
int ChooseExtractIndex_(const deque<int>& reference_string, const deque<int>& frame, int loaded_index)
{
	int index = 0;
	...
	{
		int offset = FindFutureOffset(reference_string, loaded_index, page_id);
		if (offset == -1)
			return index;
...
int FindFutureOffset(const deque<int>& feild, int start_index, const int finding_elem)
{
	...
	for (start_index; start_index< feild.size(); start_index++)
	{
		if (feild[start_index] == finding_elem)
